    ...
    public OfficeVisit build(final OfficeVisitForm ovf) {
        ...
        try {
            ...
            at = AppointmentType.GENERAL_CHECKUP; /*
                                                   * If for some reason we don't have a type, default to general checkup
                                                   */
        }
        ov.setType(at);
            ...
            try {
                final AppointmentRequest match = requests.stream().filter(e -> e.getDate().equals(ov.getDate()))
                        .collect(Collectors.toList()).get(0); /*
                                                               * We should have one and only one appointment for the
                                                               * provided HCP & patient and the time specified
                                                               */
        ...
        if (ovf.getDiagnoses() != null) {
            ov.setDiagnoses(ovf.getDiagnoses().stream().map(diagnosisService::build).collect(Collectors.toList()));
